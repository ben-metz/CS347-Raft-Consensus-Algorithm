...
export interface IServerStatusVisualisationButtonProps {
  serverId: number;
  className?: string;
}

const ServerStatusVisualisationButton: FC<IServerStatusVisualisationButtonProps> = ({
  serverId,
  className,
}) => {
  ...
  return (
    <Button onClick={onClick} className={[getButtonClass(serverStatus), className ?? ''].join(' ')} disabled={disabled}>
      {getMessage(serverStatus)} Server {serverId}
    </Button>
  );
}

export default ServerStatusVisualisationButton;
